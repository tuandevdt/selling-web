...
export default function CheckoutPage() {
  ...
  const handleCreateOrder = async () => {
    try {
      ...
      // Gọi API tạo thanh toán nếu chọn VN Pay
      if (paymentMethod === 'vnpay') {
        const params = new URLSearchParams({
          final_price: order.body.data.final_price,
          order_id: order.body.data.id
        });
        const paymentRes = await fetch(`http://localhost:8080/api/v1/payments/create-payment?${params.toString()}`, {
          method: 'GET',
          headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('access_token')}`
        ...
        // Nếu có url thanh toán, chuyển hướng sang đó
        if (paymentData?.url) {
        ...
        <Card title="Thông tin người đặt" className="bg-white shadow mb-6">
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{user.address || 'Chưa cập nhật'}</span>
              </div>
            </div>
          </Card>
